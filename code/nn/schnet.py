...
def load_dataset(fold_number):
    ...
    for fold_n, (train_ind, valid_ind) in enumerate(kfold.split(moles)):
        if fold_n != fold_number:
            continue
        
        train_moles = sorted(list(np.array(moles)[train_ind]))
        valid_moles = sorted(list(np.array(moles)[valid_ind]))
...
train, valid, test, train_moles, valid_moles, test_moles = load_dataset(FOLD_NUM)
...
def coupling_converter(batch, device):
    ...
    with_target = 'scalar_coupling_constant' in batch[0]['targets'].columns
...
class TypeWiseEvaluator(Evaluator):
    ...
    def evaluate(self):
        ...
        if self.is_submit:
            ...
            submit.to_csv(f'../schnet_results/fold_{FOLD_NUM}_kernel_schnet_test.csv', index=False)
        ...
        if self.is_validate:
            ...
            submit.drop_duplicates(subset='id', inplace=True)
            submit.sort_values('id', inplace=True)
            submit.to_csv(f'../schnet_results/fold_{FOLD_NUM}_kernel_schnet_valid.csv', index=False)
...
trainer.extend(
    training.extensions.snapshot(
        filename=f'../schnet_results/fold_{FOLD_NUM}_' + 'snapshot_iter_{.updater.iteration}.model'),
    trigger=(1, 'epoch')
)
