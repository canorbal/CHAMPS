...
if __name__ == '__main__':
    ...
    if debug:
        ...
        n_folds = 5
        n_estimators = 2000
        result_filename = '../../results/feature_selection/ase_features_result.csv'

    train = pd.read_csv('../../data/train.csv', nrows=nrows)
    ase_feats = pd.read_csv('../../data/ase_train_feats.csv', nrows=nrows)

    print('assert checking...')
    for col in ['molecule_name']:
        assert (train[col] == ase_feats[col]).sum() == len(train)

    train = concat_dataframes(train, ase_feats)

    if debug:
        train.to_csv('../../data/debug_data/ase_merge.csv', index=False)
