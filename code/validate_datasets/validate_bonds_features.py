...
if __name__ == '__main__':
    ...
    if debug:
        ...
        n_folds = 5
        n_estimators = 2000
        result_filename = '../../results/feature_selection/bonds_features_result.csv'

    train = pd.read_csv('../../data/train.csv', nrows=nrows)
    bonds_feats = pd.read_csv('../../data/bonds_train.csv', nrows=nrows)
    bonds_feats = bonds_feats.drop(['fc', 'sd', 'pso', 'dso'], axis=1)

    print('assert checking...')
    for col in ['molecule_name', 'atom_index_0', 'atom_index_1']:
        assert (train[col] == bonds_feats[col]).sum() == len(train)

    train = concat_dataframes(train, bonds_feats)

    if debug:
        train.to_csv('../../data/debug_data/bonds_merge.csv', index=False)
    ...
    categorical_cols = ['type', 'atom_1', 'atom_0']
